...
class MeatloafD64 extends MeatloafFileSystem {
	public $dos_version;
	public $disk_name;
	public $disk_id;
	public $dos_type;
	...
	protected $block_allocation_map;
	protected $geos = false;

	function __construct( $image )
	{
		$this->directory_header = Array( 18, 0 );	// Track, Sector
		$this->directory_list = Array( 18, 1 );		// Track, Sector
		// BAM Data: Track, Sector, Offset, Start Track, End Track, Byte Count
		$this->block_allocation_map = Array( [18, 0, 0x04, 1, 35, 4] ); 
...
*/
	...
	function sendListing() 
	{
		...
		{		
			...
			$rel_track = ord(fread($this->fp, 1));
			$rel_sector = ord(fread($this->fp, 1));
			$rel_length = ord(fread($this->fp, 1));
			$geos_type = ord(fread($this->fp, 1));
			$year = ord(fread($this->fp, 1));
			$month = ord(fread($this->fp, 1));
			$day = ord(fread($this->fp, 1));
			$hour = ord(fread($this->fp, 1));
			$minute = ord(fread($this->fp, 1));
			...
			switch ( $file_type & 0b00000111 )
			{
				...
				case 0x06:
					$type = "DIR"; // Parition or sub-directory
					//$hide = true;
					break;
			}
...
/*			
			echo "Next Track: $next_track\r\n";
			echo "Next Sector: $next_sector\r\n";
...
*/
		...
		} while ( $next_track !== 0x00 and $next_sector !== 0xFF );
